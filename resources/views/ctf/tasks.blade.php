...
@section('content')
  ...
  <section class="py-3">
    <div class="container px-4 px-lg-5">
      ...
      @endif
      <div class="row row-cols-3 @if(Auth::user()->role == 'admin') row-cols-md-3 @else row-cols-md-5 @endif mx-md-5 px-lg-5">
        @foreach ($tasks as $task)
        <div class="col px-1 mb-3">
          <div class="card p-0 @if(Auth::user()->taskAnswer($task->id)) success @endif">
            <div class="card-body">
              <p class="card-text m-0">{{$task->category}}</p>
              <p class="card-title m-0">{{$task->points}}</p>
              ...
              <div class="d-flex align-items-end justify-content-between">
                ...
                @if(Auth::user()->role == 'admin')
                  <div class="d-flex justify-content-end">
                    @if(!$task->trashed())
                    <a href="{{route('admin.tasks.edit_form', $task->id)}}" class="btn btn-success">Ред</a>
                    @endif
                    @if($task->trashed())
                      <form action="{{route('admin.tasks.restore', $task->id)}}" method="post">
                        @csrf
                        <button class="btn btn-warning ml-3" type="submit">Вос</button>
                      </form>
                    @else
                      <form action="{{route('admin.tasks.delete', $task->id)}}" method="post">
                        @csrf
                        @method('DELETE')
                        <button class="btn btn-danger ml-3" type="submit">Удл</button>
                      </form>
                    @endif
                  </div>
                @endif
