...
@section('content')
  <div class="px-5 my-5 container">
      ...
      <form action="{{route('user.edit')}}" method="post">
        ...
        <div class="form-group row">
          ...
          @for($key = 0; $key < 5; $key++)
            ...
            <div class="col-sm-9 mt-3">
              <input name="mem{{$key}}" type="text" class="form-control" id="mem{{$key}}"
                     value="{{$user->getMembers()[$key]??''}}">
            </div>
          @endfor
    ...
    @if($user->id == Auth::user()->getAuthIdentifier())
      @if(file_exists('files/cert/' . Auth::user()->name . '.jpg') || file_exists('files/cert/`' . Auth::user()->name . '`.jpg'))
        <div class="mt-5">
